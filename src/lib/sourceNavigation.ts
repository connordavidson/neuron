...
export const SPECIAL = /^(?:introduction|prologue|epilogue|preface|foreword|afterword|conclusion)$/i;
...
export const EXPLICIT = new RegExp('^(chapter|habit|part|book)\\s+(' + NUMBER + ')(?:\\s*[:.\\-–—]\\s*(.*))?$', 'i');
...
export function tocPages(pages: string[]): Set<number> {
...
export type SourceChapterEvidence = SourceChapter & {
  sourceStart: number;
  sourceEnd: number;
  source: 'outline' | 'heading' | 'toc';
  confidence: number;
  evidence: string[];
};

// Keep raw line/source locations alongside normalized matching text. Fresh
// layouts and navigation-only repair can map the same evidence independently.
export function collectChapterEvidence(
  pages: string[], outlines: OutlineItem[] = [], pageLineFonts: number[][] = [],
): SourceChapterEvidence[] {
  return detectSourceChapters(pages, outlines, pageLineFonts).map(chapter => {
    const lines = (pages[chapter.pageIndex] ?? '').split('\n');
    const sourceStart = lines.slice(0, chapter.lineIndex).reduce((offset, line) => offset + line.length + 1, 0);
    const sourceEnd = sourceStart + lines.slice(chapter.lineIndex, chapter.endLineIndex).join('\n').length;
    const outlined = outlines.some(item => item.pageIndex === chapter.pageIndex && headingKey(item.title) === headingKey(chapter.title));
    const confirmed = chapter.endLineIndex > chapter.lineIndex;
    return { ...chapter, sourceStart, sourceEnd,
      source: outlined ? 'outline' : EXPLICIT.test(clean(chapter.title)) || SPECIAL.test(clean(chapter.title)) ? 'heading' : 'toc',
      confidence: confirmed ? 0.94 : 0.56,
      evidence: [outlined ? 'PDF outline destination' : 'source heading or contents evidence',
        confirmed ? 'matching title at source destination' : 'destination needs confirmation'],
    };
  });
}
